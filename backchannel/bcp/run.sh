...
# data
datadir=/bls/backchanneldb/swbd_bc
# swbd_bc (etri version)
#  |_ data/
#  |_ scripts/
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    echo "stage 0: SWBD-BC Data Preparation"
    for x in $train_set $train_dev $recog_set; do
        local/data_prep.sh $datadir data/${x}
    done
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # special tokens: [sil],[lau],[spn],[nsn],[mono-bc],[dialog-bc],[non-bc]
    spm_train --input=data/lang_char/input.txt --user_defined_symbols='[unk],[sil],[lau],[spn],[nsn],[mono-bc],[dialog-bc],[non-bc]' --character_coverage=1.0 \
