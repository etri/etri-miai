...
class ContextualBlockTransformerEncoder(AbsEncoder):
    ...
    def forward_train(
        ...
        if self.block_size == 0 or total_frame_num <= self.block_size:
            ...
            block_masks = xs_pad.new_ones(xs_pad.size(0), 1, xs_pad.size(1))
            block_masks = block_masks * masks
            return xs_pad, olens, None, block_masks
        ...
        # copy output
        # first step
        block_masks = xs_pad.new_zeros(bsize, block_num, xs_pad.size(1))
        ...
        block_masks[:, block_idx, left_idx:cur_hop] = 1
        left_idx += self.hop_size
        block_idx += 1
        # following steps
        while left_idx + self.block_size < total_frame_num and block_idx < block_num:
            ...
            block_masks[:, block_idx, cur_hop : cur_hop + self.hop_size] = 1
            cur_hop += self.hop_size
            left_idx += self.hop_size
            block_idx += 1
        ...
        block_masks[:, block_idx, cur_hop:total_frame_num] = 1
        ...
        olens = masks.squeeze(1).sum(1)
        block_masks = block_masks * masks
        return ys_pad, olens, None, block_masks
    ...
    def forward_infer(
        ...
        Returns:
            position embedded tensor and mask
        """
        # retrieve previous encoder states, t-1 timestep
        ...
        # mel feature를 입력 mel_feature인 xs_pad의 앞에 이어 붙임.
        # buffer_before_downsampling은 t-1에서 쓰고 남은 mel feature들임.
        ...
        if is_final:
            ...
            # 240202 github에서 삭제되었음
            # 240611 yseokchoi 추가
            n_samples = xs_pad.size(1) // self.subsample - 1
            if n_samples < 2:
                raise TooShortUttError(
                    message="too short frame.", actual_size=xs_pad.size(1), limit=8
                )
        else:
            n_samples = xs_pad.size(1) // self.subsample - 1
            # final이 아닌데, 너무 짧은 특수한 상황.
            # final이 아닌데, 왜 n_samples가 이렇게 작은 상황이 있는지?
            if n_samples < 2:
                next_states = {
            ...
            # final이 아닌 일반적인 상황.
            # block에 해당하는 xs_pad를 만들고,
            # 남는 것들은 buffer_before_downsampling으로 보냄.
        ...
        if n_processed_blocks == 0 and total_frame_num <= self.block_size and is_final:
            ...
            return xs_pad, None, None, block_masks
        ...
        block_masks = xs_pad.new_zeros(xs_pad.size(0), block_num, y_length)
        if n_processed_blocks == 0:
            ys_pad[:, 0:offset] = ys_chunk[:, 0, 0:offset]
            block_masks[:, 0, 0:offset] = 1
        for i in range(block_num):
            cur_hop = i * self.hop_size
            if n_processed_blocks == 0:
                cur_hop += offset
            ...
            block_masks[:, i, cur_hop : cur_hop + chunk_length] = 1
        if self.normalize_before:
            ys_pad = self.after_norm(ys_pad)
        ...
        return ys_pad, None, next_states, block_masks
