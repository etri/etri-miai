...
from bcp.bc.encoder.contextual_block_conformer_encoder import (  # noqa: H301
    ContextualBlockConformerEncoder,
)
from bcp.bc.encoder.contextual_block_transformer_encoder import (  # noqa: H301
...
class Speech2TextStreaming:
    ...
    def __init__(
        ...
        # 1. Build ASR model
        scorers = {}
        asr_model, asr_train_args = BCTask.build_model_from_file(
        ...
        label_list = asr_model.label_list
        scorers.update(
            decoder=decoder,
            ctc=ctc,
        ...
        if len(label_list) > 0:
            label_converter = TokenIDConverter(
                token_list=["<blank>", "<unk>"] + label_list
            )
        else:
    ...
    def apply_frontend(
        ...
        if is_final:
            ...
            """
            # https://github.com/espnet/espnet/issues/4807 bug fixed
    ...
    def __call__(
        ...
        if feats is not None:
            enc, _, self.encoder_states, block_masks = self.asr_model.encoder(
                feats,
                feats_lengths,
                self.encoder_states,
                is_final=is_final,
            ...
            if self.asr_model.classifier is not None and block_masks is not None:
                out = self.asr_model.classifier(enc, block_masks)

                # logits = torch.nn.functional.log_softmax(out, dim=-1)
                logits = torch.nn.functional.softmax(out, dim=-1)
                pred_probs, pred_idx = torch.max(logits, dim=-1)
                pred_probs = pred_probs.squeeze(0)
                pred_idx = pred_idx.squeeze(0)
                self.bc_outputs.extend([x.item() for x in pred_idx])
                self.bc_probs.extend([x.item() for x in pred_probs])

            if self.asr_model.decoder is not None:
        ...
        bc_outputs = self.assemble_bcs(self.bc_outputs, self.bc_probs)
        if is_final:
            self.reset()
        return ret, bc_outputs
    ...
    def assemble_bcs(self, hyps, probs):
        # remove blank symbol id, which is assumed to be 0
        token_int = hyps

        # Change integer-ids to tokens
        # token = self.label_converter.ids2tokens([x + self.asr_train_args.classifier_conf["output_size"] for x in token_int])
        token = self.label_converter.ids2tokens([x + 2 for x in token_int])

        results = (token, token_int, probs)
...
def inference(
    ...
    loader = BCTask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=BCTask.build_preprocess_fn(speech2text.asr_train_args, False),
        collate_fn=BCTask.build_collate_fn(speech2text.asr_train_args, False),
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            try:
                if sim_chunk_length == 0:
                    ...
                    for i in range(len(speech) // sim_chunk_length):
                        results, bc_results = speech2text(
                            speech=speech[
                ...
                if len(bc_results[0]) == 0:
                    bc_results = [["NOBC"], [0], [1.0]]
                else:
                    for item, add_item in zip(bc_results, ["NOBC", 0, 1.0]):
            ...
            writer["bc_int"][key] = " ".join(map(str, bc_results[1]))
            writer["bc"][key] = " ".join(bc_results[0])
            writer["bc_probs"][key] = " ".join(
                map(str, [round(x, 2) for x in bc_results[2]])
            )
